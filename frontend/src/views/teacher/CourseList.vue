...
import type { CourseVO, CourseCreateDTO, CourseUpdateDTO, CoursePageQuery, ClassVO } from '@/types/api'
...
async function fetchData() {
  loading.value = true
  try { const r = await listCourses(query); records.value = r.records; total.value = r.total }
...
async function openEdit(row: CourseVO) {
  ...
  } catch { /* classIds stays empty if fetch fails */ }
}

async function handleSubmit() {
  try { await formRef.value?.validate() } catch { return }
  try {
    const data: CourseCreateDTO = { name: formValue.name, description: formValue.description || undefined, coverUrl: formValue.coverUrl || undefined, classIds: formValue.classIds?.length ? formValue.classIds : undefined }
    if (editingId.value) { await updateCourse(editingId.value, data); message.success('更新成功') }
    else { await createCourse(data); message.success('创建成功') }
    showModal.value = false; fetchData()
  } catch (e: any) { message.error(e.message || '操作失败') }
}

function handleDelete(row: CourseVO) {
  dialog.warning({ title: '确认删除', content: `确定删除课程「${row.name}」吗？`, positiveText: '删除', negativeText: '取消',
    onPositiveClick: async () => { try { await deleteCourse(row.id); message.success('已删除'); fetchData() } catch (e: any) { message.error(e.message || '删除失败') } },
  })
}
...
<template>
  <div class="page">
        ...
        <NGi v-for="c in records" :key="c.id">
          <CourseCard :course="c" @enter="goDetail">
            <template #actions="{ course }">
              ...
              <NButton size="tiny" quaternary @click="openEdit(course)"><template #icon><NIcon :size="14"><CreateOutline /></NIcon></template></NButton>
              <NButton size="tiny" quaternary @click="handleDelete(course)"><template #icon><NIcon :size="14"><TrashOutline /></NIcon></template></NButton>
            </template>
          </CourseCard>
        </NGi>
      </NGrid>
    ...
    <NModal v-model:show="showModal" :title="modalTitle" preset="card" style="width:480px">
      <NForm ref="formRef" :model="formValue" :rules="rules" label-placement="left" label-width="72">
        ...
        <NFormItem label="授课班级">
          <NSelect v-model:value="formValue.classIds" :options="allClasses.map(c => ({ label: c.grade + '级' + c.name, value: c.id }))" multiple placeholder="选择班级（可多选）" clearable />
        </NFormItem>
      </NForm>
