Create Codex pet row `running-right` for `holmes`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Holmes is a foolish, plump, round blue shorthair cat mascot with tiny paws, chubby cheeks, sleepy confused eyes, a small dumb smile, compact whole-body silhouette, blue fur with darker blue ear tips and tail, no props.. Preserve silhouette, face, palette, material, proportions, markings, and props.
